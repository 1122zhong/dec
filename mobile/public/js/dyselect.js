(function(doc,win,page_width,font_size){
    ...
    if(!doc.addEventListener)
        return;win.addEventListener(resizeEvt,function(){
        recalc();
    },false);
...
function selectSwiper(obj){
    var _self=this;_self.el=$(obj.el);
    ...
    _self.swiperData.init=obj.init;var hgSelect=`
        <div class="select">
            <span class="close" style="margin-left: 1rem;margin-top: 0.5rem;">取消</span>
            <span class="ok" style="margin-right: 1rem;margin-top: 0.5rem;">确定</span>
    ...
    `;_self.init=function(){
        ...
        _self.selectSwiper=new Swiper(obj.el+' .swiper-container',
            {direction:'vertical',
                ...
                for(i=0;i<data.length;i++){s[i+1]='<div class="swiper-slide">'+data[i]+'</div>';
                }
    ...
    _self.openSelectSwiper=function(){
        var _self=this;_self.el.addClass('yes');
        _self.selectSwiper.update();
        _self.selectSwiper.slideTo(
            _self.swiperData.activeIndex+1,0);
    };
    _self.okSelectSwiper=function(){
        if(_self.swiperData.mustSelect&&_self.swiperData.activeIndex===-1){
            ...
            _self.swiperData.oldIndex=_self.swiperData.activeIndex;_self.closeSelectSwiper();
        }};
